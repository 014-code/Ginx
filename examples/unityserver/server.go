...
// Run 是 Unity 示例入口，账号编号和连接初始化均由示例负责。
func Run() {
	...
	unityServer.nextPlayerID.Store(1000)
	...
	server.SetOnConnStart(func(connection gface.IConnection) {
		...
		playerID := unityServer.nextPlayerID.Add(1)
		// Unity 示例自行分配临时访客编号，Token 有效期为一天。
		gameSession, _, err := unityServer.Sessions.Issue(accountID, playerID, 24*time.Hour)
		if err != nil {
		...
		if _, err := unityServer.Sessions.Bind(gameSession.Token, connection.GetConnId()); err != nil {
			_, _ = unityServer.Sessions.LogoutByToken(gameSession.Token)
			connection.Stop()
			return
		}
