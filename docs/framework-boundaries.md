...
`schema/gameapp.json` 是示例拥有的协议定义，生成到 `examples/gameapp/protocol`，
不扩展框架业务 API。`tools/protocolgen` 可以处理其他应用的同格式定义；普通 Go 构建不调用
Python，传输协议也不增加版本字段。使用和兼容性限制见 [协议工具链](protocol-toolchain.md)。
